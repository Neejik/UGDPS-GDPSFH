...
class TimeoutCheck {
    public static function NoTimeoutOrDie($ip, $code_timeout) {
        ...
        // Timeouts Config
        $timeout_levels = 60;
        $timeout_accounts = 3600;
        $timeout_comments = 60;
        $max_errors_timeout = 50;
        $global_timeout = 7200;
        #$max_request_per_day = 30;

        if($code_timeout == -700) $timeout = $timeout_levels;
        elseif($code_timeout == -701 || $code_timeout == -705) $timeout = $timeout_accounts;
        elseif($code_timeout == -702) $timeout = $timeout_comments;
        elseif($code_timeout == -703) $timeout = 86400;
		elseif($code_timeout == -706) $timeout = 604800;
        else $timeout = $global_timeout;
